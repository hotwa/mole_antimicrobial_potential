    ...
    try:
        for frame in reader:
            ...
            if chem_id_colname in frame.columns:
                selected = frame[[smiles_colname, chem_id_colname]].copy()
                selected = selected.dropna(subset=[smiles_colname]).reset_index(drop=True)
                shard = selected.rename(columns={smiles_colname: "smiles", chem_id_colname: "chem_id"})
            else:
                selected = frame[[smiles_colname]].copy()
                selected = selected.dropna(subset=[smiles_colname]).reset_index(drop=True)
                shard = selected.rename(columns={smiles_colname: "smiles"})
            ...
            table = pa.Table.from_pandas(shard[["smiles", "chem_id", "source_group"]], preserve_index=False)
