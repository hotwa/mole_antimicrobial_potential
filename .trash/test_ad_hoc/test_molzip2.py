"""Test molzip approach - version 2."""
...
# Test 1: Try molzip with AtomMapNumber
print("=== Test 1: Try molzip with AtomMapNumber ===")
scaffold = Chem.MolFromSmiles(scaffold_smiles)
print(f"Scaffold: {Chem.MolToSmiles(scaffold)}")

# Create fragment with atom map
pos = "pos4"
label = POSITION_LABELS[pos]
fragment = label_fragment_with_atommap(fragment_smiles[pos], label)
...
# Test 2: Try with a simpler scaffold
print("\n=== Test 2: Try with a simpler scaffold ===")
...
params2.label = rdmolops.MolzipLabel.Isotope
try:
    result2 = rdmolops.molzip(scaffold_simple, fragment_simple, params2)
    print(f"Molzip result2: {Chem.MolToSmiles(result2)}")
except Exception as e:
    print(f"Molzip failed: {e}")

# Test 3: Try with atom map number
print("\n=== Test 3: Try with atom map number ===")
...
params3.label = rdmolops.MolzipLabel.AtomMapNumber
try:
    result3 = rdmolops.molzip(scaffold_map, fragment_map, params3)
    print(f"Molzip result3: {Chem.MolToSmiles(result3)}")
except Exception as e:
    print(f"Molzip failed: {e}")

# Test 4: Check what the scaffold looks like
print("\n=== Test 4: Check scaffold atoms ===")
scaffold = Chem.MolFromSmiles(scaffold_smiles)
